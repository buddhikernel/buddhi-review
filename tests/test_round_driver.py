...
def test_cosmetic_only_round_that_pushed_nothing_stays_parked_and_ends_the_run():
    # THE CONTROL for the pushed case above: the fixer reports applied but the
    # tree is clean, so nothing is pushed. Nothing new exists for the reviewer to
    # see — it stays parked as polish-only, no verification round is requested,
    # the run ends in round 1 and merges on the review it already has.
...
def test_dismissed_substantive_reviewer_is_reviewed_no_change(capsys):
    ...
    # and never the polish mislabel; round 2's carries the same drop.
...
def test_rr_active_cosmetic_only_responder_stays_parked_when_nothing_was_pushed():
    # The control for the pushed case above, on the restart path: the same shape
    # on a clean tree (nothing to push) keeps the polish rule exactly as before —
    # parked, NEVER re-requested, the run finishes in round 1.
    class CleanTreeGh(GhRecorder):
        def __call__(self, argv, *, cwd=None, timeout=None):
            self.calls.append(list(argv))
            if list(argv)[:3] == ["git", "status", "--porcelain"]:
